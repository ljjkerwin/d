...
function exec(animation, percentage, key) {
	...
	if ( isObject(startValue) && isObject(endValue) ) {
		nextValue = { };
...
function intervalByFrame(fn){
	requestAnimationFrame(() => {
		fn();
		if(timerId) intervalByFrame(fn);
	});
}
...
export default {
	...
	add: function(options) {
		...
		queue.push(Object.assign({}, options, {
			easing: easing,
			id: taskId,
		...
		if (!timerId){
			timerId = Date.now();
			intervalByFrame(runAnimationQueue);
		}
